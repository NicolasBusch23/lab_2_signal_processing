...
def caracteristicas_a(datos, F_s):
    ...
    datos = datos - np.mean(datos) # Elimina el componente DC para un análisis de frecuencia apropiado
    n = len(datos)

    # Revisar en MATLAB
    frec = np.fft.rfftfreq(n, d = 1/F_s)  # Frecuencias de la FFT
    fft = np.abs(np.fft.rfft(datos)) # FFT
    espectro_energia = fft**2 # Densidad espectral de energía

    # Para buscar el índice con valor máximo en el espectro de energía
    indice_max = np.argmax(espectro_energia)
        
    # Para extraer la frecuencia dominante alrededor de la cual está centrada la energía del fenómeno
    f_dom = frec[indice_max]

    # Para extraer la magnitud del espectro de energía asociada a f_dom
    energia_f_dom = espectro_energia[indice_max]
    ...
    plt.plot(f_dom, energia_f_dom, 'ro', label=f'F. Dom: {f_dom:.2f} Hz')
    ...
    return media, sd, rms, maximo, minimo, rango, energia, f_dom, energia_f_dom
...
def ppm(datos, F_s):
    ...
    periodo_estimado, valor_max_autocorr = p1.calculo_maximo(l, autocorr, F_s)

    if periodo_estimado == np.nan or valor_max_autocorr == np.nan:
        ppm = np.nan
        print("--> Fallo en cálculo de autocorrelación y ppm")
    else:
